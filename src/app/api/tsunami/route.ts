...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'start':
        url = `${BACKEND_URL}/api/tsunami/start-tsunami`;
        break;
      ...
      case 'next_round':
        ...
        url = `${BACKEND_URL}/api/tsunami/next-round`;
      ...
      case 'vote':
        ...
        url = `${BACKEND_URL}/api/tsunami/vote-best-deception`;
    ...
    return NextResponse.json(
      { error: 'Failed to process tsunami request' },
      { status: 500 }
    );
  }
}